  ...
  const renderRelatedPost = (relatedPost) => {
    ...
    return (
      <Link
        ...
        className="mt-2 p-2 bg-gray-50 rounded-md flex max-w-full sm:max-w-36 items-center space-x-2 hover:bg-gray-100 transition-colors"
      >
        {relatedPost.image && (
          <img src={relatedPost.image} alt="Post preview" className="w-10 h-10 object-cover rounded" />
        )}
        <div className="flex-1 overflow-hidden">
          <p className="text-sm text-gray-600 truncate max-w-12">
            {relatedPost.content}
          </p>
        </div>
          ...
          {isLoading ? (
            ...
            <ul className="space-y-4">
              {notifications.data.map((notification) => (
                ...
                >
                  <div className="flex items-start justify-between sm:flex-row flex-col space-y-4 sm:space-x-4">
